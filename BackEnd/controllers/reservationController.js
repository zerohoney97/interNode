...
exports.checkReservedList = async (req, res) => {
    ...
    try {
        ...
        const reservedList = await ReservedList.findOne({ where: { reservation_num, seat_num, show_id, user_id} });
        if (reservedList) {
            result= true;
        }
    ...
    // io.on("connection", async (socket) => {
        ...
        socket.on("payment", async (data) => {
            try {
                ...
                // 유저가 예매했다고 예매내역 추가
                await ReservedList.create({ reservation_num, seat_num, show_id, user_id});
