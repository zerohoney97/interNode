// 닉네임, 이메일, 패스워드 정규식에 따라 true/false 변경하기
// 닉네임, 이메일, 패스워드
// let nickPass = false; // 닉네임 정규식에 맞는지
...
window.onload = async () => {
    ...
    duplicatedBtn.onclick = async () => {
        ...
        try {
            const { data } = await axios.post("http://127.0.0.1:8080/signUp/checkemail", email, {
    ...
    sendMailBtn.onclick = async () => {
        ...
        if (emailCheckPass) {
            ...
            try {
                const { data } = await axios.post("http://127.0.0.1:8080/signUp/sendemail", email, {
                    withCredentials: true,
                });
                if (data == "0") {
                    alert("인증코드를 보냈습니다.");
    ...
    codeBtn.onclick = async () => {
        if (codeInput.value == "") {
            ...
            try {
                const { data } = await axios.post("http://127.0.0.1:8080/signUp/checkcode", emailCode, {
                    withCredentials: true,
                });
                if (data == "0") {
                    // 인증코드 일치
    ...
    signUpBtn.onclick = async () => {
        ...
        if (emailPass && emailDupPass && emailCheckPass) {
            try {
                const user = { email : emailInput.value, password : passwordInput.value, nickname: nicknameInput.value };
                const {data} = await axios.post("http://127.0.0.1:8080/signUp/", user, {
                    withCredentials: true,
                });
                if (data == "0") {
                    alert("회원가입 완료");
